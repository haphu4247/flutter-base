...
class DiskCacheStore extends ICacheStore {
  ...
  Future<CacheObj?> getCacheObj(String key, {String? subKey}) async {
    ...
    if (null != subKey) {
      where += ' and $_columnSubKey="$subKey"';
    }
    final resultList = await db.query(_tableCacheObject, where: where);
    if (resultList.isEmpty) {
      return null;
    }
    return _decryptCacheObj(CacheObj.fromJson(resultList[0]));
  }
  ...
  Future<bool> setCacheObj(CacheObj obj) async {
    ...
    final content = await obj.encryptContent();
    final headers = await obj.encryptHeaders();
    await db.insert(
        _tableCacheObject,
        {
          _columnKey: obj.key,
  ...
  Future<bool> _clearExpired(Database? db) async {
    ...
    final where1 = '$_columnMaxStaleDate > 0 and $_columnMaxStaleDate < $now';
    final where2 = '$_columnMaxStaleDate <= 0 and $_columnMaxAgeDate < $now';
    return 0 !=
